...
for file_i = 1:length(folder_names)
    ...
    if plot__peakLatency
        ...
        for ijk = 1:2
            ...
            fileName = [fig_suffix 'HeatMap_dff_' num2str(after_puff) '.png'];
            ...
            figure('Position', [360, 278, 600, 250], 'Visible', 'on'); hold on
            axis off;
            axes('YAxisLocation', 'left', ...
                'Color', 'none', ...
                'YColor', 'none', 'XColor', 'k'); hold on
            ...
            print(['rangeOf_' fig_suffix num2str(after_puff) '.png'], '-dpng', '-r300')
            ...
            fileName = ['distOf_' fig_suffix num2str(after_puff) '.png'];
            fullFilePath = fullfile(folderPath, fileName);
            print(fullFilePath, '-dpng', '-r300');

        end

        fileName = ['SampleTraces_OnsetPeak.png'];
        fullFilePath = fullfile(folderPath, fileName);
        print(fullFilePath, '-dpng', '-r300');
...
fileName = ['peak_latency_separate.png'];
fullFilePath = fullfile(folderPath, fileName);
print(fullFilePath, '-dpng', '-r300');
...
group_labels = [repmat({'Animal 1'}, length(time_to_peak.Nigel), 1); ...
                ...
                repmat({'Animal 3'}, length(time_to_peak.Bill), 1); ...
                repmat({'Animal 4'}, length(time_to_peak.Jeremy), 1)];
h = boxplot([time_to_peak.Nigel'; time_to_peak.Bernie'; time_to_peak.Bill'; time_to_peak.Jeremy'], group_labels, 'colors', 'k');
