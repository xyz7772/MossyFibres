...
% Plot everything 
if manual
    figure('Position',[100,50,300,250]), hold off, plot(xx,yy,'-','Color',[.5,.5,.5]), hold on
    ...
    figure('Position',[100,50,300,250]), hold off, histogram(X*v,'Normalization','pdf','FaceColor',[.5,.5,.5]), hold on
    plot(-.5:.01:.5,normpdf(-.5:.01:.5,mean_proj_baseline,sqrt(var_proj_baseline)),'r','LineWidth',2)
    set(gca,'FontSize',15)
    xlim([-.5,.5])
    ylabel('PDF')
