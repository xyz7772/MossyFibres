...
%  Copyright (c) 2000-2022 Andrew Knyazev, Rico Argentati
%  Contact email: <Andrew.Knyazev@ucdenver.edu>
%  License: free software (BSD)
%  $Revision: 4.6 $  $Date: 2021/12/20
threshold=sqrt(2)/2; % Define threshold for determining when an angle is small
if size(F,1) ~= size(G,1)
    error('MATLAB:subspacea:rowDimentionMismach',...
        ['The row dimension ' int2str(size(F,1)) ...
...
if nargin<3  % Compute angles using standard inner product
    ...
    if ~isstr(A)
        [mAc,mA] = size(A);
        if mAc ~= mA
            error('MATLAB:subspacea:matrixNotSquare',...
                'Matrix A must be a square matrix or a string.')
        end
        if mA ~= m
            error('MATLAB:subspacea:matrixVectorDimentionMismach',...
                ['The size ' int2str(size(A)) ...
...
if ~isstr(A)
    [mAc,mA] = size(A);
    if mAc ~= mA
        error('MATLAB:subspacea:ortha:matrixNotSquare',...
            'Matrix A must be a square matrix or a string.')
    end
    if mA ~= m
        error('MATLAB:subspacea:ortha:matrixVectorDimentionMismach',...
            ['The size ' int2str(size(A)) ...
